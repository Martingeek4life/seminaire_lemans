...
def get_yo_pretrained_word_embedding(word):
    # Charger le tokenizer et le modèle
    tokenizer = BertTokenizer.from_pretrained('Davlan/bert-base-multilingual-cased-finetuned-yoruba')
    model = BertModel.from_pretrained('Davlan/bert-base-multilingual-cased-finetuned-yoruba')
    ...
    # Obtenir les embeddings pour la dernière couche
    embeddings = outputs.last_hidden_state.squeeze(0)  # Supprimer la dimension de batch

    # Moyenne des embeddings des tokens pour obtenir un vecteur unique pour le mot
    word_embedding = embeddings.mean(dim=0)

    # Convertir le tensor en liste puis en chaîne de caractères
    embedding_str = ' '.join([str(x) for x in word_embedding])
...
def generate_pretrained_w2v_it_en(words_source, words_target, model_pretrained_source, model_pretrained_target, source_yo, target_yo):
    ...
    if(len(words_source) != 0):
        with open("source_embeddings1.vec", "w", encoding="utf-8") as f_out:
            ...
            for word_it in words_source:
                try:
                    if(source_yo == 1):
                        embedding =model_pretrained_source.wv[word_it]
                    else:
...
def GMUD(before, after):
    command = [
        'python3', './GMUD.py', '--embeddings_before', before, '--embeddings_after', after, '--distance_metric', 'cosine', '--nb_neighbors', '201'
    ]
    subprocess.run(command)
...
if __name__ == "__main__":
    download_pretrained_yoruba()
    Gzip_pretrained_yoruba()
    ...
    model_pretrained_es = KeyedVectors.load_word2vec_format(hf_hub_download(repo_id="Word2vec/wikipedia2vec_eswiki_20180420_300d", filename="eswiki_20180420_300d.txt"))
    model_pretrained_de = KeyedVectors.load_word2vec_format(hf_hub_download(repo_id="Word2vec/wikipedia2vec_dewiki_20180420_300d", filename="dewiki_20180420_300d.txt"))
    ...
    source_output_path_aux = "source_embeddings_ew"
    ...
    if (args.rich_lang_source and args.rich_lang_target):
        ...
        elif args.pretrained_lang_source == "es":
            model_pretrained_source = model_pretrained_es
            print("model_pretrained_es")
        elif args.pretrained_lang_source == "de":
            model_pretrained_source = model_pretrained_de
            print("model_pretrained_de")
        elif args.pretrained_lang_source == "zh":
            model_pretrained_source = model_pretrained_zh
            print("model_pretrained_zh")
        elif args.pretrained_lang_source == "ar":
            model_pretrained_source = model_pretrained_ar
            print("model_pretrained_ar")

        elif args.pretrained_lang_source == "ru":
            model_pretrained_source = model_pretrained_ru
            print("model_pretrained_ru")
        elif args.pretrained_lang_source == "yo":
            print("model_pretrained_yo")
            model_pretrained_source = model_pretrained_yo
            source_yo = 1
        else:
        ...
        elif args.pretrained_lang_target == "es":
            model_pretrained_target = model_pretrained_es
        elif args.pretrained_lang_target == "de":
            model_pretrained_target = model_pretrained_de
        elif args.pretrained_lang_target == "zh":
            model_pretrained_target = model_pretrained_zh
            print("model_pretrained_zh")
        elif args.pretrained_lang_target == "ar":
            model_pretrained_target = model_pretrained_ar
            print("model_pretrained_ar")

        elif args.pretrained_lang_target == "ru":
            model_pretrained_target = model_pretrained_ru
            print("model_pretrained_ru")
        ...
        generate_pretrained_w2v_it_en(words_source, words_target, model_pretrained_source, model_pretrained_target, source_yo, target_yo)
        ...
        if(args.pretrained_lang_target == "en"):
            ...
            model_pretrained_target = model_pretrained_it
        generate_pretrained_w2v_it_en([], words_target, model_pretrained_fr, model_pretrained_target, source_yo, target_yo)
        ...
        if(args.pretrained_lang_source == "en"):
            ...
            model_pretrained_source = model_pretrained_it
        generate_pretrained_w2v_it_en(words_source, [], model_pretrained_source , model_pretrained_fr, source_yo, target_yo)
    else: 
        generate_word_embeddings(clean_corpus_source_path, source_output_path)
        generate_word_embeddings(clean_corpus_target_path, target_output_path)
    ...
    write_dictionary_to_file(Dictionnary, "Dico.txt")

    print("--------------------------Calcul des deformations moyenne GMUD-----------------------")
    GMUD(source_output_path, source_cross_path)
