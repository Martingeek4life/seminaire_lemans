...
def get_data_from_bible(csv_path, output_dir, data_type = "para", langues=[], livres=[], cell_error = "__Error__"):
    ...
    for i in range(l-1):
        for j in range(i+1, l):
            ...
            with open(li_abrev, 'w', encoding='utf-8') as txtfile1:
                with open(lj_abrev, 'w', encoding='utf-8') as txtfile2:
            
                    for fichier in livres:
                        try :
                            with open(csv_path+"/csvs/"+fichier+'csv', 'r', encoding='utf-8') as csvfile:
                                print(csv_path+"/csvs/"+fichier+'csv')
                                taille = os.path.getsize(csv_path+"csvs/"+fichier+'csv')
                                print("la taille du fichier est: ", taille/1024)
                                ...
                                try :
									...
									# Extraire les noms des colonnes (première ligne)
                                    fieldnames = df.columns.tolist()

                                    # Afficher les noms des colonnes
                                    print(fieldnames)
                                    print("suis entré dans le try")

                                except StopIteration :
                                    pass
                                try :
                                    print("j'entre dans index_i", fieldnames)
                                    ...
                                    for index, ligne in df.iterrows():
                                        compteur = compteur + 1
                                        print("compteur dans for: ", compteur)
                                        print(ligne)
                                        x_i = ligne[index_i] 
                                        y_i =  ligne[index_j]
                                        if x_i != cell_error and y_i != cell_error :
                                            ...
                                            errors = errors + 1
                                    print("len ligne: ", compteur)
                                except Exception as e:
                                    print("except", e.encode('utf-8'))
                                    pass
                        except :
                            pass
