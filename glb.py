...
def compute_pca_axes(vectors):
    ...
    pca.fit(centered)
    # axes échelle par racine de la variance
    axes = pca.components_ * np.sqrt(pca.explained_variance_).reshape(3,1)
    return mean, axes

def plot_space_with_directors(vectors, words, color='blue', title='Espace', filename='space.png'):
    ...
    # Affichage des vecteurs directeurs (axes principaux) comme grandes flèches
    for i in range(3):
    ...
    # Ellipsoïde pour la forme globale
...
if __name__ == "__main__":
    ...
    plot_space_with_directors(vectors_before_3d, words_before, color='blue', title='Espace avant', filename='espace_avant_vecteurs.png')

    # Figure pour espace après avec vecteurs directeurs en rouge
    plot_space_with_directors(vectors_after_3d, words_after, color='red', title='Espace après', filename='espace_apres_vecteurs.png')
